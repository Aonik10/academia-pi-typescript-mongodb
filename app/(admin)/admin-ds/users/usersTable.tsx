    ...
    return (
        <td className="p-0">
            <div className="position-relative p-2">
                <a href={"/admin-ds/users/" + id} className="stretched-link"></a>
                {content}
            </div>
        </td>
    );
...
function TableRow({ user }: TableRowProps) {
    ...
    return (
        <tr>
            ...
            <TableData id={user._id} content={user.phoneNumber} />
            <TableData id={user._id} content={user.id_document} />
            <TableData id={user._id} content={user.inscriptions.length} />
            <TableData id={user._id} content={user.reffersCodes.length} />
        </tr>
    );
}
...
function SearchBar({
    ...
    onClick,
    onKeyPress,
}: InputComponentProps<HTMLInputElement>) {
    return (
...
export default function UsersTable({ users }: UsersTableProps) {
    ...
    const handleFilter = async () => {
        ...
        if (response.users) {
            setUsersRendered(response.users)
        }
                ...
                <div className="d-flex justify-content-center ">
                    <ul className="pagination">
                        ...
                        <li
                            ...
                            onClick={() =>
                                setPage(
                                    Math.floor(usersRendered.length / pageSize)
                                )
                            }
                        >
                            {"»"}
